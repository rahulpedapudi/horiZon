...
const ROLES = [
  { id: "student", label: "Student", icon: GraduationCap, description: "I'm currently studying." },
  { id: "trader", label: "Trader / Investor", icon: TrendingUp, description: "I trade or invest in markets." },
  { id: "business", label: "Business / Startup", icon: Building2, description: "I run or plan to start a business." },
  { id: "career_switcher", label: "Career Switcher", icon: RefreshCcw, description: "I'm looking to pivot my career." },
  { id: "learner", label: "Lifelong Learner", icon: BrainCircuit, description: "I learn for personal growth." },
];
...
const CURIOSITY_SUGGESTIONS = [
  "AI", "Machine Learning", "Web3", "Cybersecurity", 
  "Robotics", "Data Science", "Cloud Computing"
];
    ...
    steps: [
      {
        ...
        type: "single",
        options: ["Engineering", "Medical (MBBS / Allied)", "Science / Arts / Commerce", "Government Exam Aspirant", "Other"],
        customOption: true
      },
      {
        id: "branch",
        ...
        options: ["CSE / IT", "ECE", "EEE", "Mechanical", "Civil", "Chemical", "Other"]
      },
      {
        id: "skills",
        ...
        suggestions: ["HTML", "CSS", "JavaScript", "React", "Python", "Java", "DSA", "ML", "Cloud"]
      }
    ]
  },
  trader: {
    steps: [
      {
        ...
        type: "single",
        options: ["Stock Market", "Crypto", "Forex", "Commodities", "Long-term Investing"]
      },
      {
        id: "experience",
      ...
      },
      {
        ...
        suggestions: ["Technical Analysis", "Risk Management", "Options", "Macro Economics", "Psychology"]
      },
      {
        id: "trading_goal",
        title: "Trading Goal",
        ...
        type: "single",
        options: ["Side income", "Full-time trading", "Long-term wealth", "Knowledge mastery"]
      }
    ]
  },
  business: {
    steps: [
      {
        ...
        type: "single",
        options: ["Idea stage", "Early startup", "Running business", "Aspiring entrepreneur"]
      },
      {
        id: "domain",
        ...
        type: "single",
        options: ["Tech / SaaS", "E-commerce", "Local business", "Manufacturing", "Services", "Creator economy"]
      },
      {
        id: "skill_stack",
        title: "Skill Stack",
        ...
        suggestions: ["Marketing", "Sales", "Finance", "Product", "Operations", "Leadership", "AI", "Automation"]
      },
      {
        id: "time_horizon",
        title: "Time Horizon",
    ...
    steps: [
      {
        ...
        type: "single",
        options: ["Engineering", "Non-Engineering", "Commerce", "Arts", "Other"]
      },
      {
        id: "target_direction",
      ...
      },
      {
        ...
        type: "skills_with_exposure",
        suggestions: ["Communication", "Analysis", "Coding", "Management", "Domain knowledge"]
      }
    ]
  },
  learner: {
    steps: [
      ...
      },
      {
        ...
        suggestions: ["AI", "Technology", "Economics", "Psychology", "Science", "Philosophy"]
      }
    ]
  }
};
...
const Onboarding = () => {
  ...
  const getActiveFlowSteps = () => {
    ...
    const roleSteps = currentFlow.steps.filter(s => s.condition ? s.condition(formData) : true);
  ...
  const renderContent = () => {
    ...
    if (flowStepIndex >= 0 && flowStepIndex < activeFlowSteps.length) {
                       ...
                       <div className="space-y-2">
                           {EXPOSURE_LEVELS.map((level) => (
                               <div 
                                 ...
                                 `}
                               >
                                   <span className="text-sm font-medium text-gray-900 dark:text-gray-200">{level}</span>
                                   {formData.exposure_level === level && (
      ...
      if (config.type === "curiosity") {
          return (
              <StepContainer step={step} total={totalSteps} title={config.title} subtitle={config.subtitle}>
                  <ChipInput 
                      value={value || []}
                      inputValue={chipInput}
                      setInputValue={setChipInput}
              ...
              <div className="grid gap-3">
                {config.options.map((opt) => (
                  <SelectionCard
                    key={opt}
                    label={opt}
                    selected={config.type === "multi" ? (value || []).includes(opt) : value === opt}
                    onClick={() => {
    ...
    if (step === totalSteps) {
      return (
        <div className="animate-fade-in max-w-lg mx-auto text-center space-y-8 py-10">
            ...
            <div className="space-y-2">
                <h2 className="text-3xl font-bold text-gray-900 dark:text-white">You're ready for Horizon</h2>
                <p className="text-gray-500 dark:text-gray-400">Here is your setup profile.</p>
            </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-6 text-left space-y-4 border border-gray-200 dark:border-gray-700">
                <SummaryItem label="Role" value={ROLES.find(r => r.id === role)?.label} />
                ...
                <SummaryItem label="Exposure Level" value={formData.exposure_level} />
            </div>
  ...
  const isNextDisabled = () => {
      ...
      if (flowStepIndex >= 0 && flowStepIndex < activeFlowSteps.length) {
          ...
          if (config.type === "curiosity" || config.type === "preferences" || config.type === "multi") {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 flex flex-col font-sans text-gray-900 dark:text-white">
      ...
      <div className="px-6 py-6 flex justify-between items-center max-w-5xl mx-auto w-full">
          ...
          <button onClick={handleSkip} className="text-sm font-medium text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors">
              Skip Setup
          </button>
      </div>
              ...
              {step < totalSteps && (
                  <div className="flex items-center justify-between mt-12 pt-6 border-t border-gray-100 dark:border-gray-900">
                      <button 
                        onClick={handleBack} 
                        disabled={step === 1}
                        className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium transition-colors ${step === 1 ? 'invisible' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-900'}`}>
...
const StepContainer = ({ step, total, title, subtitle, children }) => (
    <div className="animate-slide-up">
        <span className="text-sm font-semibold text-gray-400 mb-2 block tracking-wide uppercase">Step {step} of {total - 1}</span>
        <h1 className="text-3xl md:text-4xl font-bold mb-3 tight-heading">{title}</h1>
        <p className="text-lg text-gray-500 dark:text-gray-400 mb-8 max-w-lg">{subtitle}</p>
        {children}
    </div>
);

const SelectionCard = ({ label, icon: Icon, description, selected, onClick, multi }) => (
    <div 
      onClick={onClick}
      className={`
    ...
    >
        {Icon && (
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${selected ? 'bg-black text-white dark:bg-white dark:text-black' : 'bg-gray-100 dark:bg-gray-900 text-gray-500 group-hover:bg-gray-200 dark:group-hover:bg-gray-800'}`}>
                <Icon size={24} strokeWidth={1.5} />
            </div>
        )}
        <div className="flex-1">
            <h3 className={`font-semibold text-lg ${selected ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>{label}</h3>
        ...
        {multi && (
             <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${selected ? 'bg-black border-black dark:bg-white dark:border-white' : 'border-gray-300 dark:border-gray-600'}`}>
                  {selected && <Check size={14} className="text-white dark:text-black" strokeWidth={3}/>}
             </div>
        )}
    </div>
);

const ChipInput = ({ value, inputValue, setInputValue, onAdd, onRemove, suggestions, placeholder }) => (
   <div className="space-y-6">
       ...
       <div className="flex flex-wrap gap-2 mt-4">
           {value.map((chip) => (
               <div key={chip} className="flex items-center gap-2 px-4 py-2 bg-black dark:bg-white text-white dark:text-black rounded-full text-sm font-medium animate-fade-in">
                   {chip}
                   <button onClick={() => onRemove(chip)} className="hover:text-gray-300 dark:hover:text-gray-600">
                       <X size={14} />
                   </button>
               </div>
           ))}
